# Hi there 👋, I'm Joseph Arbolino

This is a collection of all the projects and exercises I completed or collaborated on. I have tried to make this github as user friendly as possible and have included as many explanations, pictures, examples, and directions for execution as I could. Most of these projects were completed during my college career and therefore include many files needed for the execution of the program but that I didn't write myself. These files were all interfaced with during the completion of the projects so I do understand how all the code works and what its purpose is, but it would be wrong of me to pass these on as my own work. This is also the reason GitHub reports the language varience to be ~70% Python. Most of the supporting files provided for us were written in Python so the actual split between the languages I actively practice and code in is more even. Below is a table of contents (of a sort) giving a breif overview of what each folder in this repository contains. There will be more information provided in each folder, if you have any questions please do not hesitate to contact me with the details found in the "Contact Information" section.

Thanks for stopping by!!
#

### Arduino

### Artificial Intelligence

### Computer Systems

### Data Structures and Algorithms

### Embedded Systems

### IEEE Robotics 2022

### Large Scale Software Development

### Software Reverse Engineering


<!--
**joe-arbo/joe-arbo** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=joe-arbo&count_private=true&include_all_commits=true&show_icons=true&theme=tokyonight&text_color=FFFFFF)

&hide=contribs,prs,stars

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
